import { Player } from './player.js?v=v367';
import { Level } from './level.js?v=v367';
import { Enemy, GelChaser, TractorUFO, SweeperUFO } from './enemies.js?v=v367';
import { UIManager } from './ui.js?v=v367';
import { CloudSaveManager } from './cloud_save.js?v=v367';
import { audio } from './audio.js?v=v367';
import { LevelEditor } from './editor.js?v=v367';
import { Boss, CyberBoss, UfoBoss } from './boss.js?v=v367';
...
export class GameManager {
    constructor(canvasId) {
        ...
        const tutorialStars = this.getStarsForLevel(0);
        const hasFinishedTutorial = tutorialStars > 0 || this.unlockedLevel > 1;
        this.currentLevel = (savedLvl !== null && hasFinishedTutorial) ? this.unlockedLevel : 0;
    ...
    respawnAtCheckpoint() {
        ...
        // Eğer henüz bir checkpoint'e değmemişse (spawn noktasındaysa) tam canla, 
        // eğer checkpoint'ten canlanıyorsa 1 canla başlasın.
        const isAtSpawn = (this.checkpointX === this.level.spawnX && this.checkpointY === this.level.spawnY);
    ...
    _showWinScreen() {
        ...
        // Save level record locally for campaign levels
        if (this.currentLevel !== 999) {
            try {
                let records = JSON.parse(localStorage.getItem('viscora_level_records') || '{}');
                const lvlKey = 'level_' + this.currentLevel;
                const existingRecord = records[lvlKey] || {};
                const bestTime = existingRecord.bestTime ? Math.min(existingRecord.bestTime, this.gameTime) : this.gameTime;
                const bestCrystals = Math.max(existingRecord.crystalsCollected || 0, collectedCrystals);
                records[lvlKey] = {
                    bestTime: bestTime,
                    crystalsCollected: bestCrystals,
                    crystalsTotal: totalCrystals
                };
                localStorage.setItem('viscora_level_records', JSON.stringify(records));
            } catch(e) {}
        }
    ...
    draw() {
        ...
        this.ctx.fillText('v367', this.cssWidth - 10, 10);
