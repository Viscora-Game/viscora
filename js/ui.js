...
import { ViscosityList } from './viscosity.js?v=v395';
import { shopManager, SHOP_ITEMS } from './shop.js?v=v395';
...
export class UIManager {
    ...
    initGoogleSignInButton() {
        ...
        const renderFallbackGoogleBtn = (container) => {
            if (!container) return;
            container.innerHTML = `
                <button type="button" class="custom-google-btn" style="display: inline-flex; align-items: center; justify-content: center; gap: 8px; background: #ffffff; color: #1f2937; border: 1px solid #d1d5db; border-radius: 8px; padding: 7px 14px; font-family: sans-serif; font-size: 0.82rem; font-weight: 600; cursor: pointer; box-shadow: 0 2px 5px rgba(0,0,0,0.2); transition: all 0.2s ease;">
            ...
            try {
                ...
                if (container1) {
                    ...
                    window.google.accounts.id.renderButton(container1, {
                        theme: 'dark', size: 'medium', type: 'standard', shape: 'rectangular', text: 'signin_with', logo_alignment: 'left'
                    });
                }
                if (container2) {
                ...
                // GIS render edemediyse (örn Native WebView ortamında) 400ms sonra fallback buton yerleştir
                setTimeout(() => {
                    if (container1 && (!container1.querySelector('iframe') || container1.children.length === 0)) {
                        renderFallbackGoogleBtn(container1);
                    }
                    if (container2 && (!container2.querySelector('iframe') || container2.children.length === 0)) {
                        renderFallbackGoogleBtn(container2);
                    }
                }, 400);
            } catch (e) {
    ...
    isLevelUnlocked(lvlNum) {
        ...
        // Zorunlu Eğitim Kontrolü (Yeni oyuncular için Seviye 1'i kilitle)
        const unlockedLvl = this.game ? this.game.unlockedLevel : (parseInt(localStorage.getItem('viscora_unlocked_level')) || 1);
        const tutorialStars = this.game ? this.game.getStarsForLevel(0) : (parseInt(JSON.parse(localStorage.getItem('viscora_stars') || '{}')[0]) || 0);
        if (lvlNum >= 1 && unlockedLvl <= 1 && tutorialStars === 0) {
            return false;
        }
