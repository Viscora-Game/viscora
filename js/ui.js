...
import { ViscosityList } from './viscosity.js?v=v313';
import { shopManager, SHOP_ITEMS } from './shop.js?v=v313';
import { CloudSaveManager } from './cloud_save.js?v=v313';
            ...
            const drawCardPreview = (item, canvas) => {
                ...
                if (item.category === 'accessory' && item.id !== 'default_accessory') {
                    ...
                    if (item.id === 'cowboy_hat') {
                        ...
                        ctx.lineWidth = 2.5;
                        ctx.beginPath();
                        ctx.arc(cx, cy - 3, radius - 1, Math.PI * 0.9, Math.PI * 0.1, true);
                        ctx.stroke();
                        ...
                        // Yarı saydam neon vizör camı
                        const visorGrad = ctx.createLinearGradient(cx - 11, cy - 6, cx + 11, cy);
                        ...
                        ctx.beginPath();
                        if (ctx.roundRect) {
                            ctx.roundRect(cx - 11, cy - 6.5, 22, 7.5, 3.5);
                        } else {
                            ctx.rect(cx - 11, cy - 6.5, 22, 7.5);
                        }
                        ctx.fillStyle = visorGrad;
                        ctx.fill();
                        ...
                        ctx.lineWidth = 0.5;
                        ctx.beginPath();
                        ctx.moveTo(cx - 9, cy - 2.8);
                        ctx.lineTo(cx + 9, cy - 2.8);
                        ctx.stroke();
                        ctx.restore();
    ...
    initProfileUI() {
        ...
        if (avatarPicker) {
            ...
            this.profileAvatars.forEach(av => {
                ...
                img.src = `assets/avatars/${av.id}.png?v=v313`;
        ...
        const updateWidget = () => {
            ...
            if (widgetAvatar) {
                widgetAvatar.src = `assets/avatars/${currentAvatar}.png?v=v313`;
            }
        };
